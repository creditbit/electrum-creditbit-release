...
get_archpkg (){
  ...
  wget https://aur.archlinux.org/packages/en/electrum-creditbit-git/electrum-creditbit-git.tar.gz
  tar -xpzvf electrum-creditbit-git.tar.gz
  sed -e 's/_gitbranch\=.*/_gitbranch='${archbranch}'/g' electrum-creditbit-git/PKGBUILD > electrum-creditbit-git/PKGBUILD.new
  mv electrum-creditbit-git/PKGBUILD.new electrum-creditbit-git/PKGBUILD
  rm electrum-creditbit-git.tar.gz
  popd
}
prepare_repo(){
...
buildRelease(){
  ...
  # echo "Making locales" 
  # $DOCKERBIN run --rm -it --privileged -e MKPKG_VER=${VERSION} -v $(pwd)/helpers:/root  -v $(pwd)/repo:/root/repo  -v $(pwd)/source:/opt/wine-electrum/drive_c/electrum-creditbit/ -v $(pwd):/root/electrum-creditbit-release creditbit/electrum-creditbit-release:${VERSION} /bin/bash
...
buildImage(){
  echo "Building image"
  case "${1}" in 
  winbuild) $DOCKERBIN build -t creditbit/electrum-creditbit-winbuild:${VERSION} .
         ;;
   release) $DOCKERBIN build -f Dockerfile-release -t  creditbit/electrum-creditbit-release:${VERSION} .
         ;;
  esac
}
...
prepareFile(){
  ...
  if [ -e "$TARGETPATH" ]; then
   ...
   wget https://github.com/creditbit/electrum-creditbit/archive/v${VERSION}.zip -O $TARGETPATH
  fi
